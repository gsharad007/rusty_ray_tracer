...
mod tests_ppm {
    ...
    #[test]
    fn get_stride() {
        assert_eq!(3 * 2, PPM::new(2, 2, 255, vec![]).get_stride());
        assert_eq!(3 * 8, PPM::new(8, 8, 255, vec![]).get_stride());
        assert_eq!(3 * 16, PPM::new(16, 16, 255, vec![]).get_stride());
        assert_eq!(3 * 32, PPM::new(32, 32, 255, vec![]).get_stride());

        assert_eq!(3 * 1024, PPM::new(1024, 32, 255, vec![]).get_stride());
        assert_eq!(3 * 2048, PPM::new(2048, 32, 255, vec![]).get_stride());
        assert_eq!(3 * 4096, PPM::new(4096, 32, 255, vec![]).get_stride());
    }
